...
const TableDetails = ({ communityCards = [], pot = 0 }) => {
  const displayCards =
    communityCards.length > 0 ? communityCards : [null, null, null, null, null];
    ...
    <>
        ...
        {displayCards.map((card, index) => {
          if (card) {
            ...
            return (
              <div
                key={index}
                className="w-12 h-16 sm:w-14 sm:h-20 md:w-16 md:h-24"
              >
                <CardBack />
              </div>
            );
      ...
      <div className="absolute top-[35%] left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className="bg-black/50 text-white px-2 py-1 md:px-4 md:py-2 rounded-full text-center">
          ...
          <span className="font-bold text-sm md:text-xl ml-1 md:ml-2">
            ${pot}
          </span>
        </div>
      </div>
