...
export default function Seat({
  ...
  if (!seat.player) {
    return (
      <>
        <div className={`absolute ${seatStyle} transform z-10 pointer-events-auto`}>
          <div className="bg-white/20 backdrop-blur-sm p-2 md:p-3 rounded-lg shadow-lg flex flex-col items-center">
            <button 
              className={`px-3 py-1 rounded-md text-sm font-medium ${
                isInGame ? "bg-gray-500 cursor-not-allowed text-gray-300" : "bg-blue-600 hover:bg-blue-700 text-white"
              }`}
              onClick={openModal}
              disabled={isInGame}
            >
            ...
            <div className="text-white text-xs mt-1">Seat {seat.seatNumber}</div>
          </div>
        </div>
        ...
        <dialog ref={modalRef} className="modal modal-bottom sm:modal-middle">
          <div className="modal-box bg-gray-800 rounded-lg shadow-xl border border-gray-700">
            <h3 className="text-white text-lg font-bold">Select Buy-In Amount</h3>
            ...
            <>
              <div className="w-10 md:w-14">
                {seat.player.handCards && seat.player.handCards.length > 0 ? (
                  <Card card={`${seat.player.handCards[0].value}${seat.player.handCards[0].suit[0]}`} />
                ) : (
                  <CardBack />
                )}
              </div>
              <div className="w-10 md:w-14">
                {seat.player.handCards && seat.player.handCards.length > 1 ? (
                  <Card card={`${seat.player.handCards[1].value}${seat.player.handCards[1].suit[0]}`} />
                ) : (
                  <CardBack />
                )}
              </div>
